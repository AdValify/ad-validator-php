...
class AdValify {
    ...
    private function callAPI($uri, $method, $arr_data=[]) {
        if($this->endpoint == 'ENTER_YOUR_ENDPOINT_HERE') {
            $this->err = "Endpoint not set. Open AdValify.php and enter your endpoint (sent by email).";
            return false;
        }
        if($this->endpoint == 'ENTER_YOUR_API_KEY_HERE') {
            $this->err = "API Key not set. Open AdValify.php and enter your API Key (sent by email).";
            return false;
        }
        
        $url = rtrim($this->endpoint,'/').$uri;
        $ch = curl_init($url);
          curl_setopt($ch, CURLOPT_USERAGENT, "AdValify SDK v3.0");
